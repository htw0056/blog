...
Spring框架中bean可以有多种作用域：`singleton`，`prototype`，`session`，`request`等，在绝大多数情况下，`singleton`作用域就能满足使用需求了。但是如果我们想把Prototype Beans注入到Singleton Instance中，使得每次调用Singleton实例的方法获取Prototype Beans都是不同的实例，那该如何操作呢？
...
1. `getPrototypeBean()`每次获取到的`PrototypeBean实例`都是一样的，因为返回的是代理对象
2. 在实际调用PrototypeBean方法时，该代理才会委托给实际对象来执行（有可能是新生成实例，也有可能复用已有的对象）

我们通过测试类来看下以上两种不同点，导致输出的结果有何不同：
...
>  [Injecting Spring Prototype bean into Singleton bean](http://dolszewski.com/spring/accessing-prototype-bean-in-singleton)一文也对该问题进行了解释，可供参考
